...
int main(int argc, char *argv[])
{
  ...
  TextWriter out(Config::outputDir + "/" +
                 "fs-AP-k" + to_string(k) + "_H" + Config::hubType + "_DEP" + to_string(Config::depth) + "_STP" + to_string(Config::stopRea) + "_H" + to_string(Config::numHubs) + "_E" + to_string(Config::eta));
  ...
  for (int i = result.size() - 1; i >= 0; i--)
  {
    ...
    out.writeln(to_string(npv.n1) + "\t" + to_string(npv.n2) + "\t" + to_string(npv.val));
    count++;
    if (count == k)
      break;
  }
